...
import {LoadType} from '../../../shop/product/shipping-estimator/shipping-estimator.component';
import {OrderItem} from '../../../../shared/model/orderItem.model';
import {ShippingService} from '../../../../core/service/shipping.service';

@Component({
  selector: 'app-select-shipping-modal',
  templateUrl: './select-shipping-modal.component.html',
  styleUrls: ['./select-shipping-modal.component.css']
})
export class SelectShippingModalComponent implements OnInit {
  ...
  submit(): void {
    this.dirty = true;
    if (this.shippingCategory === 'CONTAINERS' && this.units.valid) {
      // do something
      this.getQuotes();
    } else if (this.shippingCategory === 'PALLETS-BOXES' && this.units.valid) {
  ...
  getQuotes(): void {
    ...
    if (this.shippingCategory === 'PALLETS-BOXES') {
      if (this.loadType === this.loadTypeEnum.boxes) {
        this.shippingQuote.quantity = this.orderItem.cartItem.quantity;
        this.shippingQuote.width = this.orderItem.cartItem.product.information.width;
        this.shippingQuote.height = this.orderItem.cartItem.product.information.height;
        this.shippingQuote.length = this.orderItem.cartItem.product.information.length;
        this.shippingQuote.weight = this.orderItem.cartItem.product.information.weight;
      }
      else if (this.loadType === this.loadTypeEnum.pallets) {
  ...
  selectMethod(): void {
    if (this.selectedMethod !== -1) {
      this.orderItem.shippingQuote = this.shippingQuote;
      this.orderItem.shippingMode = this.ratesResponse.mode[this.selectedMethod];
